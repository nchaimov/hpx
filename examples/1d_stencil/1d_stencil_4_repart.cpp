...
struct stepper
{
    ...
    // time steps
    hpx::future<space> do_work(std::size_t np, std::size_t nx, std::size_t nt, double * data)
    {
...
int hpx_main(boost::program_options::variables_map& vm)
{
    ...
    for(boost::uint64_t i = 0; i < nr; ++i) {
        ...
        hpx::future<stepper::space> result = step.do_work(parts, size_per_part, nt, data);
